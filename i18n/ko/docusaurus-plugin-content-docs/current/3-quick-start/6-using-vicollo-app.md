...
#### 1. 멤버가 직접 Vicollo 앱에 로그인하여 사용하게 하기

Vicollo 앱에 멤버를 등록하고, 해당 멤버가 앱의 웹페이지를 통해 직접 이용하도록 합니다.
구성이 간단하지만, 멤버 활동을 세밀하게 통제하기는 어렵습니다.

#### 2. 서버에서 멤버 사용을 제어 아래 사용하게 하기

기존 서비스와 Vicollo 기능을 통합하려는 경우에 적합합니다.
이 경우 사용자는 비디오룸에 직접 참여하는 부분만 Vicollo의 웹페이지에서 이루어지며 다른 Vicollo의 기능들은 여러분의 서비스를 통해서 제공되도록 하는 것이 일반적입니다. 이를 위해서 여러분 서비스의 멤버를 Vicollo 앱에 등록하여, 사용자들의 비콜로에 대한 기능들을 서비스 서버에서 Vicollo App Server REST API를 사용해 대리해 수행하도록 해야 하며 이 과정에서 멤버의 활동을 관리할 수 있게 됩니다.
...
Vicollo 앱 생성을 통한 사용을 처음 계획하셨다면 여러분의 요구사항에 따른 추가 개발이 이루어질 수 있기 때문에 실제 서비스의 운영 환경인 production 환경이 아닌 sandbox 환경에서 테스트 환경이 구성되어 동일한 정보의 별도의 계정이 제공될 수 있습니다. Sandbox 환경에 여러분의 Vicollo 앱을 생성함에 따라 production 환경과 분리된 환경에서 행여나 있을 수 있는 여러분의 요구사항에 대한 추가 개발이 이루어졌을 때 테스트 하실 수 있게 되며 요청하신 기능이 안정적으로 동작하게 되면 추후에 production 환경에 적용되게 됩니다.

비콜로 앱 생성과 사용 승인이 완료되면 다음 과정을 진행합니다:

1. `Vicollo.Live`(<https://vicollo.live>)에 로그인 (sandbox 환경 계정이 제공 되었을 경우 <https://sandbox.vicollo.live>에서 로그인)
2. 우측 상단의 **App** 버튼 클릭하여 앱 관리 화면으로 이동
3. **Create App** 선택
4. 필요한 정보를 입력하여 앱 생성
...
Vicollo 앱의 구체적인 설정 방법과 그에 따른 운영 방법에 대한 설명에 앞서 전반적인 기능들에 대한 이해를 돕기 위해 Vicollo 앱을 경험해 보는 과정을 살펴보겠습니다.
